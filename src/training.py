...
#
# This module includes the training logic of Alpha Zero
#
######################################################
...
def execute_self_play(game_settings, mcts_settings, policy):
    """
...
class AlphaZeroTraining:
    ...
    def net_compet(self, gen):
        ...
        if compet_freq != 0 and (gen + 1) % compet_freq == 0:
            ...
            scores = 0
            p1_params = {
                "policy_path": self.temp_policy_path,
                "nn_training_settings": self.nn_training_settings,
            }
            p2_params = {
                "policy_path": self.config.nn_training_settings.policy_path,
                "nn_training_settings": self.nn_training_settings,
            }
            ...
            for round_n in range(compet_rounds):
                if round_n > compet_rounds / 2 - 1:
                    reversed = True
                match_params = {
                    "player1": [net_player, p1_params],
                    "player2": [net_player, p2_params],
