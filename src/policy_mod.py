...
class Policy(nn.Module):
    ...
    def nn_train (self, replay_buffer, batch_size):
        ...
        for i in range(training_steps): 
            ...
            loss = self.loss_function(torch.Tensor(all_states), pred_v, pred_p, target_v, target_p)
            loss.backward()
    ...
    def loss_function(self, input_board, pred_v, pred_p, target_v, target_p):
        ...
        self.loss_records.append(np.array([value_loss.sum(), - policy_loss.sum(), constant.sum()]) / pred_p.size()[0])
